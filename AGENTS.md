...
| `agend-core` 沒有 build script、沒有 `[features]`；唯一直接依賴是停用預設功能且只開 `derive` + `alloc` 的 `serde`（D32，第 1 關提案 P7） | `cargo xtask check-deps`（`cargo metadata`） |
...
- 已確認的決策：[docs/DECISIONS.md](docs/DECISIONS.md)（D1–D32）。
