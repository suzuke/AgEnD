...
| `agend-core` 沒有 build script、沒有 `[features]`；依賴 allowlist 原本為空，第 1 關草稿暫時放入停用預設功能且只開 `derive` + `alloc` 的 `serde`，**待使用者確認 P7**（[gate-01-core.md](docs/gates/gate-01-core.md#p7core-依賴-allowlist)）；否決則 allowlist 回到空 | `cargo xtask check-deps`（`cargo metadata`） |
