...
| `tests/server.rs` 連線 | 第二個 `Spawn` 回 `already_spawned` 且不動 agent；新連線接手、舊的被關；版本不合回錯誤、holder 照常服務；落後 1 MiB 被斷線 |
| `tests/server.rs` 終端查詢 | bash 送 `ESC[6n`，讀回 holder 的 `ESC[1;1R` |
| `tests/server.rs` 停止 | 忽略 HUP 的 agent 在 5 秒後被 SIGKILL；一般 agent 很快停；process group 不留任何程序 |
