...
| `git_fixture::tests` | 真的 git：canonical、origin、worktree、branch 建得出來，worktree 上的 commit 不動 `main`，drop 後目錄刪掉；`command` 拒絕暫存目錄外的目錄（`/`、`<tmp>`、`root/..`、相對路徑）與 `../x` 檔名；fixture 放在外層 repo 裡時，任何指令都碰不到外層 repo（`root()` 本身拒絕，其他目錄的 `rev-parse` 都落在 `root()` 裡）；`commit` 拒絕路徑上的 symlink，外面什麼都沒寫；`git` 拒絕 `-C`／`--git-dir`／`--work-tree`／`--namespace`／`-c`；繼承的 `GIT_*`／`AGEND_*` 全部移除、全域與系統設定關掉；FRG-10 的觀察在真 git 上成立（正常 merge 保留之前的 merge，`update-ref` 把 main 設成 branch head 就丟掉） |
