...
| `driver::codex::send::tests`（P6） | 兩個競態（用照稿回答的連線，假 app-server 沒辦法照時機重現）：`queue/add` 回覆時已閒置 → `queue/start` 一次、`-32600` 算成功，還在忙就不呼叫；steer 撞上剛結束的 turn（`-32600`）→ 一次 `turn/start`，其他錯誤不重試；閒置時三級都是 `turn/start` |
...
- [ ] 排隊的訊息已經開始成為一個 turn、但它的 user message 還沒出現在歷史時 daemon 重連：對帳兩邊都找不到，會再送一次（第 7 施工關 K-項，見頁面）
