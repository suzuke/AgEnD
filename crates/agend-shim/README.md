...
2. 解析全域選項與子命令；讀不到 cwd（被刪掉了）→ 拒絕（`cwd_unreadable`，同真 git 的 `Unable to read current working directory`），不拿 `.` 代替；不用 cwd 的呼叫（`--version`、`init`、`clone`、第一個 `-C` 是絕對路徑）照常往下
3. 位置：用呼叫者的全域選項、cwd、`GIT_*` 跑 `git rev-parse --absolute-git-dir --git-common-dir --show-toplevel --show-prefix`，分成綁定的 worktree／canonical checkout／同 repo 的其他 worktree（含 canonical 與別的 worktree 的 submodule）／綁定 worktree 裡的巢狀 repo（submodule）／外部 repo／自己的 workspace（`<AGEND_HOME>/workspace/<instance>` 或它底下，比對 canonical 路徑，不在 repo、沒帶 `-C`／`--git-dir`／`--work-tree`／`GIT_*`）／其他不在 repo 的地方／不知道（快照壞）；`--version`、`init`、`clone` 不問
...
7. 讀取 → 綁定且在 canonical checkout，或在自己的 workspace（`<AGEND_HOME>/workspace/<instance>` 或它底下）而沒帶 `-C`／`--git-dir`／`--work-tree`／`GIT_*`，就導向並印出 `running in your bound worktree` 那行；否則原地放行（`-C` 打錯字、`/tmp/x` 這類不在 repo 的目錄由 git 自己報錯）
8. 寫入 → 要有效快照與綁定；在綁定的 worktree 裡：git 回答的 work tree 是它、`GIT_INDEX_FILE` 在它的 git dir 裡；不在 repo 而且不是自己的 workspace（`/tmp/x`、`~`，或呼叫者用 `-C`／`--git-dir`／`--work-tree`／`GIT_*` 指定的地方 git 找不到 repo）時拒絕（`no_repo_there`，照 git 的 `not a git repository`，不改成在整個 worktree 跑）；需要導向時：不是從別的 worktree、不在 git dir 裡、沒有 `GIT_*`、沒有 `--git-dir`／`--work-tree`、同一個子目錄在 worktree 裡存在而且不是 submodule／巢狀 repo；worktree 有 hook；`checkout`／`switch` 不離開綁定的 branch；不是 `branch` 複製或改名、`symbolic-ref` 寫入、`reflog delete|expire`、`stash` 寫入、`git remote` 寫入（`remote_write`）、autostash、`clean -x|-X|-ff`、recurse 進 submodule、`submodule foreach`；破壞性操作先快照
9. 導向 = 真 git 加 `-C <worktree>/<prefix>`、拿掉呼叫者的 `-C`／`--git-dir`／`--work-tree`，一律印出 `running in your bound worktree` 那行；之後 ref 的變更由 hook 檢查
