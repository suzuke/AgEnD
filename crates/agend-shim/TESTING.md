...
| `classify::tests::*` | 全域選項解析；導向（保留子目錄、worktree 沒有的目錄與別的 worktree 拒絕；`-C`／`--git-dir`／`--work-tree`／`--bare`／`GIT_*` 指的地方、或不是 workspace 的目錄沒有 repo 時寫入拒絕、讀取原地跑；導向都印出 note）；讀取放行；未綁定／快照壞時拒絕寫入；沒裝 hook 拒絕寫入；`core.hooksPath` 與 `push --no-verify` 拒絕；`worktree`；`checkout`／`switch` 離開 branch（含縮寫與 `-bfoo`）、`branch -c/-C/-m/-M` 各種拼法拒絕（`-fm`、`--mo`、`--cop`；`-u<值>` 不誤判）、`symbolic-ref` 寫入與 `reflog delete|expire` 拒絕（讀取放行）、`-`／`@{-N}` 回到自己的 branch、`checkout refs/heads/<自己的>` 拒絕（會 detach）；`stash` 寫入與 `clean -x|-X`（含 `-fdx`、`-fX`，`-e<值>` 不誤判）拒絕、`stash list|show` 放行；`clean -ff` 各種拼法（`-fdf`、`--force --force`、`--forc`）拒絕、`clean -fd -eff` 不誤判；有 `.gitmodules` 時 recurse 進 submodule（旗標與縮寫、`-c`、config）的破壞性 `reset`／`checkout`／`restore`／`switch` 拒絕，`--no-recurse-submodules`、`merge`／`rebase`、沒有 `.gitmodules` 不誤判；`submodule foreach` 拒絕；`git remote` 寫入（`add`、`-v add`、`remove`／`rm`、`rename`、`set-url`、`set-head`、`set-branches`、`prune`、`update --prune|-p|--pru`）從 worktree、canonical、workspace 拒絕，`remote`、`-v`、`get-url`、`show`、`update`、`prune --dry-run|-n|--dry` 放行；worktree 裡的巢狀 repo：寫入原地跑、破壞性的快照、team clone 照樣拒絕；autostash 拒絕（`--autostash` 與縮寫、`-c`／`--config-env`／`GIT_CONFIG_*` 不分大小寫、config 檔設了而沒帶 `--no-autostash`；`--continue`／`--abort` 這類動作與 `merge --edit`、`--autosquash` 不誤判）；work tree／index 必須是綁定的；快照範圍（含縮寫 `reset --har`、`mv -f`）；不認得的子命令；外部 repo 與 team repo；日常命令 |
...
| `shim_everyday.rs` | 第 4 輪：正常工作清單（含 `git push`、`push -u origin <branch>`、`checkout -`、`stash list`），hook 有跑；刻意拒絕的（含 `stash push`／`pop`）附下一步；symlink 與空格路徑 8 種拼法組合；第 11 輪 `git remote` 的 `set-url`／`remove`／`rename`／`add`／`update --prune` 從 worktree、canonical、workspace 拒絕，canonical 的 remote 與 `refs/remotes/origin/main` 不變，`remote -v`／`get-url`／`show`／`update` 照常 |
| `shim_route_scope.rs` | 第 5 輪：導向保留子目錄（repro E／B）、別的 worktree 拒絕、worktree 沒有的目錄拒絕；第 8 輪 `clean -x|-X` 各種拼法從 worktree、canonical、子目錄都拒絕，`.env`、`target/` 還在，`clean -fd` 照常且不動 ignored 檔；第 10 輪 `-C typo` 的 `reset --hard`、`checkout .`、`restore .`、`clean -fd`、`rm -r -f .` × worktree、子目錄、canonical、workspace，加 `-C src/sbu`、`-C <不是 repo 的目錄>`、canonical 的 `-C mod`（只有 worktree 有 `mod/`）、`--git-dir`／`--work-tree`／`GIT_DIR`／`GIT_WORK_TREE` 指到沒有 repo 的地方：寫入拒絕（`no_repo_there`），修改都還在、沒快照；讀取照打的跑由 git 報錯；workspace 不帶 `-C` 照常導向；第 11 輪真的 binary 從被刪掉的目錄（`cd target/debug` 後刪掉）跑 `restore .`、`checkout .`、`clean -fd`、`rm -r -f .` × worktree、canonical、別的 worktree、巢狀 repo：拒絕（`cwd_unreadable`，同真 git 的 `Unable to read current working directory`），修改都還在、沒快照；被刪掉的 cwd 裡 `--version`、`-C <worktree 絕對路徑>` 的讀取與（快照後的）`restore` 照常；第 12 輪不帶 `-C` 從不在 repo 也不是 workspace 的目錄（fixture 外的 scratch、它的子目錄、`$AGEND_HOME` 的上層）跑 `checkout .`、`restore .`、`reset --hard`、`clean -fd`、`rm -r -q -f .`、`add .`、`commit -a`：寫入拒絕（`no_repo_there`，`not a git repository`，下一步 `cd <worktree>`），修改都還在、HEAD 不動、沒快照；讀取由 git 報錯；workspace 與它底下照常導向並印出 note；`remote prune --dry-run` 照常 |
| `shim_submodule.rs` | 第 9 輪：`submodule.recurse`／`--recurse-submodules` 的 `reset --hard`、`checkout .`、`restore .` 在 git 執行前拒絕，submodule 裡的修改還在，`--no-recurse-submodules` 照常；`git -C mod reset --hard`、`cd mod; git checkout .` 在 submodule 裡快照，照印出的那行在 `mod` 裡還原得回來；`submodule foreach` 拒絕；canonical 的 submodule 裡 `reset --hard` 拒絕；`clean -ffd`／`-fdf`／`-f -f`／`--force --force` 拒絕，巢狀 repo 與它的本機 commit 還在，`clean -fd` 照常（git 跳過巢狀 repo）；第 10 輪 canonical 裡沒初始化的 submodule（空的 `mod/`）導向會落進 worktree 的 submodule，拒絕（`route_dir_missing`），修改還在 |
...
| `shim_git.rs` | 導向、`checkout main` 拒絕並記 audit、worktree／branch 建立、未綁定與快照壞、protected ref（hook 拒絕、記 audit）、`reset --hard` 快照後照訊息還原、第 8 輪人在 canonical 的 stash 撐過 agent 每種 stash 寫入（shim 拒絕；繞過 shim 的真 git 由 hook 拒絕，有沒有 agent 環境都一樣）、autostash（使用者已決定 2026-09-25：有衝突的未提交修改時 `pull --rebase --autostash` 等在 git 執行前拒絕，HEAD、README、`refs/stash` 不變；config 設了 `rebase.autoStash`／`merge.autoStash` 時 plain `rebase`／`pull`／`merge` 一樣拒絕，`--no-autostash` 由 git 自己拒絕髒的 worktree，commit 後照常跑）、bypass 不跳過 hook、外部 repo、hook 形式的 `GIT_DIR` |
