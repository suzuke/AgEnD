...
| `classify::tests::*` | 全域選項解析；有 git dir 沒 work tree 時 cwd 必須是綁定的 worktree；導向；讀取放行；未綁定／快照壞時拒絕寫入；`worktree`；branch 切換、建立、刪除、改名；protected ref；每一類繞法：縮寫選項、隱含的 push 目的地、fetch refmap、config key、symbolic ref、work tree／index、DWIM 與旁門、外部 repo 與 team repo（用假的 `Probe`） |
...
| `team::tests::*` | URL 正規化（scp、ssh、https、埠號、`.git`）、insteadOf；本機路徑照 git 補 `.git`（`/x/origin` = `/x/origin.git`） |
