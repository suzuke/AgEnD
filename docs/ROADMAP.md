...
> - 目前狀態：**第 1 關待驗收**；每關狀態看下表，做了什麼看最下面的「進度紀錄」。
...
| 關 | 狀態 | 範圍 | 驗收（可觀察） |
|---|---|---|---|
| 1 `core` | [待驗收](gates/gate-01-core.md) | agend-core：型別、兩套協定（client + holder）、trait、流水線狀態機（6 種關卡）、busy policy、去抖動、衝突偵測、merge 門檻（patch-id）、螢幕分類器 | `cargo xtask accept core` 跑測試並印出一個模擬 task 走完 `code` workflow（純邏輯，無 daemon） |
...
7 項提案（P1–P7）與逐條確認記錄只列在 [gate-01-core.md](gates/gate-01-core.md#開工前提案)。
...
- 2026-09-25 review 修正 pipeline、assignment、protocol 相容與 check-deps 自我檢查；workspace tests（66 core tests、5 protocol compatibility tests）、clippy、check-deps、accept core 通過；待 fresh-context verifier 與使用者親自驗收（工作樹，尚未提交）。
- 2026-09-24 `agend-core` 初次自動驗收通過：workspace fmt/clippy、49 core tests、no-std check 與 demo；P1–P7 對照確認，P6 由第 5 關 Store 落地（工作樹，尚未提交）。
- 2026-09-24 使用者確認第 1 關提案 P1–P7，開始 `agend-core` 實作（工作樹，尚未提交）。
