...
| crate | 依賴 |
|---|---|
| `agend-core` | `serde`（`default-features = false`，`derive` + `alloc`；JSON I/O 在 adapter） |
...
agend-core 不用 std（`#![no_std]` + `alloc`），只有 `serde` 可供型別 derive（無預設 features，只開 `derive` + `alloc`）；JSON 編碼留在 adapter，時間只經 `Clock` trait。保護方式：
