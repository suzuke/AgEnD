...
| crate | 依賴 |
|---|---|
| `agend-core` | 第 1 關草稿：`serde`（`default-features = false`，`derive` + `alloc`；JSON I/O 在 adapter），**待 P7 確認**；否決則無依賴 |
...
agend-core 不用 std（`#![no_std]` + `alloc`）；第 1 關草稿讓 `serde` 供型別 derive（無預設 features，只開 `derive` + `alloc`），這項例外待使用者確認 P7（[gate-01-core.md](gates/gate-01-core.md#p7core-依賴-allowlist)）；JSON 編碼留在 adapter，時間只經 `Clock` trait。保護方式：
...
| `cargo metadata` 規則 | agend-core 有 build script、有任何 `[features]`、serde default features 或 derive／alloc 以外 features，或有 serde 以外的依賴（normal／build／dev）；serde 這一項待 P7 確認 | `cargo xtask check-deps` |
