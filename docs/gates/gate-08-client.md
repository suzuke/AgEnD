...
| C6 | `resolve_attention` 的 `accepted` 在項目拿掉、`attention_resolved` 發出之後**立刻**回，不等 `retry` 做完（停舊 holder 最多 15 秒） | CLI 不必卡 15 秒；結果看事件（`instance_changed … starting` → `unknown`，或再死一次又回到「需要你」） | 等 `start` 做完才回：supervisor 把 reply 往後移一行 |
