...
> - 下一步：照「你親自驗收」跑一次（「待你追認」T1–T23 已全部決定或追認）。
...
- git shim：hook 看不到的
  - 導向：綁定時從 canonical checkout、或從自己的 workspace（`<AGEND_HOME>/workspace/<instance>` 或它底下，比對 canonical 路徑；本身不在任何 repo）不帶 `-C`／`--git-dir`／`--work-tree`／`GIT_DIR`／`GIT_WORK_TREE` 跑的 git 改在綁定的 worktree 跑，保留子目錄，並印出 `running in your bound worktree` 那行；從其他不在任何 repo 的目錄（`/tmp/x`、`~`）寫入拒絕（`no_repo_there`，照 git 的 `not a git repository`，下一步 `cd <worktree>`），讀取照打的跑、由 git 自己報錯（第 12 輪）；別的 worktree 裡的寫入、worktree 沒有的目錄（或在 worktree 裡是 submodule／巢狀 repo）、git dir 裡的寫入拒絕；呼叫者指定的地方 git 找不到 repo 時（`-C` 打錯字）寫入拒絕，絕不改成在整個 worktree 跑（第 10 輪）；cwd 已經被刪掉時（`cd target/debug` 之後 `cargo clean`）讀取與寫入都拒絕（`cwd_unreadable`，照 git 的 `Unable to read current working directory`，下一步 `cd <worktree>`），絕不拿 `.` 代替（第 11 輪）；不用 cwd 的呼叫（`--version`、第一個 `-C` 是絕對路徑）照真 git 跑，寫入照一般規則（第 12 輪）；位置問真的 git（`rev-parse`，T20）
  ...
  - `git remote` 的寫入（`add`、`remove`／`rm`、`rename`、`set-url`、`set-head`、`set-branches`、`prune`、`update --prune`）拒絕（`remote_write`，第 11 輪）：remote 存在 canonical checkout 與每個 worktree 共用的 repo config，`remove` 還會刪掉 `branch.*.remote` 與 `refs/remotes/<name>/*`；`remote`、`remote -v`、`get-url`、`show`、`remote update`、`remote prune --dry-run|-n` 照常
...
- [x] `cargo test -p agend-shim`：`55 passed`（unit）；`cargo test -p agend`：`4`（argv0）+ `18`（`shim_bypass_corpus`）+ `2`（`shim_everyday`）+ `14`（`shim_git`）+ `8`（`shim_hooks`）+ `6`（`shim_location_matrix`）+ `2`（`shim_push_hook`）+ `10`（`shim_route_scope`）+ `5`（`shim_submodule`）passed；`TMPDIR` 設成新的空目錄時一樣全過
...
8. 自己動手：hook 裝在哪、binding 快照壞掉時。
   ...
   **這步在驗什麼**：你親眼看到 hook 只設在 agent worktree 自己的 `config.worktree`，而且 binding 快照壞掉時寧可拒絕也不猜；錯了的話 hook 會出現在你的 checkout，或 agent 在不知道自己綁在哪時寫錯地方。
   ...
   git status                                                                        # agend-shim: running in your bound worktree …，接著 On branch agend/t-1/fix（從 workspace 導向）
...
owner 睡覺時我自己做的決定；都可逆。每條打勾＝同意，不同意就寫在「驗收紀錄」備註。T7、T19、T21、T22、T23 是你已經決定的，其餘各條使用者已於 2026-09-25 追認；T21 之後 T3、T4、T7、T8、T10、T11、T17 改寫，T14、T15、T16 由 T21 取代。
...
- [x] T1 binding 快照格式：`$AGEND_HOME/bindings/<instance>.json`，JSON `{version: 1, instance, source_repo?, protected_refs?, binding?: {kind: "work", task_id, branch, worktree} | {kind: "review", task_id, head, worktree}}`；work branch 必須是 `agend/<task_id>/<slug>`。型別暫放 `agend_shim::binding::Snapshot`，建議第 10 施工關搬到 `agend_core::model`。（使用者已追認 2026-09-25）
- [x] T2 環境變數：`AGEND_HOME`、`AGEND_INSTANCE`（holder 注入；shim 與 hook 都讀，git 會把環境傳給 hook）；`AGEND_SHIM_BYPASS=1` 只跳過 shim（寫 audit）；`AGEND_SHIM_DEPTH` 防 PATH 迴圈。（使用者已追認 2026-09-25）
- [x] T3 建 branch：`git branch agend/<自己的 task-id>/<名稱>` 可以；其他名稱由 hook 拒絕。`checkout -b`／`switch -c` 一律由 shim 拒絕（會離開綁定的 branch），自己命名空間的也一樣；`branch -c/-C/-m/-M` 也一律拒絕（第 6 輪：hook 看不到新名稱），改用 `git branch <新> <舊>`。[pipeline.md](../architecture/pipeline.md#worktree-與-branch-生命週期) 寫「擋 `branch <new>`」，兩者有出入，請你決定。（使用者已追認 2026-09-25）
- [x] T4 不認得的子命令（git alias、外部 `git-*`）拒絕：擋 `git co main` 這類 alias（git 2.39 看不到 HEAD 換 branch）。`filter-branch`、`fast-import`、`replace` 不再特別拒絕：它們寫的 ref 由 hook 檢查。（使用者已追認 2026-09-25）
- [x] T5 team repo 用「目的地」認（沒有 hook 的 repo 由 shim 守）：跟 team 無關的 scratch repo 不管；remote 指向 team remote 或 canonical checkout 的 clone、team 的本機 remote 本身：讀取放行、寫入拒絕（`worktree` 只有 `list` 算讀取）；從任何 repo push 到 team remote 或 canonical 路徑拒絕。比對前套 `insteadOf`、正規化 URL；本機路徑照 git 找 repo 的順序（`/x/origin` = `/x/origin.git`），`file://` 不看主機名。包住 `$AGEND_HOME` 的 repo 不算。（使用者已追認 2026-09-25）
- [x] T6 導向：綁定時在 canonical checkout 跑的讀取與寫入都導向 worktree，保留子目錄（`--show-prefix`）；在自己的 workspace（`<AGEND_HOME>/workspace/<instance>` 或它底下，比對 canonical 路徑；本身不在任何 repo）只有**沒帶** `-C`／`--git-dir`／`--work-tree`／`GIT_DIR`／`GIT_WORK_TREE` 的呼叫導向，跑在 worktree 頂層；導向一律印出 `running in your bound worktree` 那行。第 12 輪：其他不在任何 repo 的目錄（`/tmp/x`、`~`、`$AGEND_HOME` 的上層）原本也當成 workspace、在整個 worktree 跑；現在寫入拒絕（`no_repo_there`，理由照 git：`not a git repository`，下一步 `cd <worktree>`），讀取照打的跑、由 git 自己報錯。worktree 沒有那個子目錄、或那個子目錄在 worktree 裡是 submodule／巢狀 repo（另一個 repo），讀取與寫入都拒絕（`route_dir_missing`）；在 canonical 的 `.git` 裡的寫入拒絕；別的 agent 的 worktree 裡寫入拒絕、讀取原地跑。第 10 輪：呼叫者用上面那些指定了地方、git 在那裡找不到 repo（例如 `git -C src/sbu checkout .` 打錯字；真 git 是 `fatal: cannot change to 'src/sbu'`），shim 原本丟掉 `-C`、在整個 worktree 跑；現在寫入拒絕（`no_repo_there`，理由照 git：`cannot change to '<dir>'` 或 `not a git repository`），讀取照打的跑、由 git 自己報錯。（使用者已追認 2026-09-25）
...
- [x] T8 快照：範圍對齊 v1 agentic-git（見[範圍](#範圍)），新增 merge／rebase／pull／cherry-pick／revert／am，拿掉 `read-tree -u`；`stash drop|clear` 拿掉（快照只存 work tree，救不回 stash；stash 寫入改由 T22 拒絕）；每次都做（不只 dirty 時）；含未追蹤、不含 ignored 檔，所以 `clean -x`／`-X` 改成拒絕（第 8 輪：`clean -fdx` 刪掉 `.env`，快照裡沒有）；submodule 與巢狀 repo 只存 gitlink，所以會 recurse 的破壞性操作、`clean -ff`、`submodule foreach` 拒絕，worktree 裡的 submodule／巢狀 repo 改在它自己快照（第 9 輪）；ref `refs/agend/snapshots/<instance>/<id>`，id = `<unix 秒>-<pid>`（同一程序第二次加 `-<n>`）；快照用的 git 不跑 hook；shim 不清舊快照；快照失敗就拒絕該操作。（使用者已追認 2026-09-25）
- [x] T9 kill（未改）：`pkill`／`killall` 拒絕；`kill` 只接受 `kill [-SIGNAL | -s SIG | -n NUM | --signal SIG] [--] <pid>...`，pid 去空白後只接受 `1..=2147483647` 且最多 10 位數；`0`、負數、名字、job spec、執行檔是 `agend` 的 pid 拒絕。（使用者已追認 2026-09-25）
- [x] T10 audit：`$AGEND_HOME/audit/shim.jsonl`，記 shim 與 hook 的拒絕、bypass、快照；hook 的紀錄 argv 是 hook 名稱與參數，理由寫出哪個 ref；不輪替。（使用者已追認 2026-09-25；audit 輪替與保留期限交給第 5／6 施工關）
- [x] T11 exit code：shim 拒絕 1、找不到真的工具 127；hook 拒絕時由 git 決定（`reference-transaction` 通常 128，`pre-push` 1）。（使用者已追認 2026-09-25）
- [x] T12 v1 的 gh 父程序例外拿掉：v2 由 daemon merge。（使用者已追認 2026-09-25）
- [x] T13 `GIT_DIR` 指向自己 worktree 的 git dir 時照常執行（git hook 會這樣呼叫）；寫入時 git 解析出的 work tree 必須是綁定的 worktree、`GIT_INDEX_FILE` 在它的 git dir 裡；需要導向的寫入帶 `--git-dir`／`--work-tree`／`GIT_*` 拒絕，不改寫。（使用者已追認 2026-09-25）
- [x] T17 `checkout` 只有一個參數、沒有 `--` 後面的路徑時，問 git 它是不是 commit：是就當切換（只有自己的 branch 放行），不是就當還原路徑並快照；只在 remote 有的 branch 會被 git 建成本機 branch，由 hook 拒絕。`checkout <x> --` 當切換。`-`／`@{-N}` 用 git 解析，回到自己的 branch 放行。（使用者已追認 2026-09-25）
- [x] T18 shell 內建的 `kill` 攔不到（含 `kill -9 -1`）；建議交給第 4 或第 6 施工關。（使用者已追認 2026-09-25：由第 4 施工關 P2（holder 忽略 TERM、setsid）與第 6 施工關（kill -9 後 `--resume` 補救、開機巡查孤兒）處理）
- [x] T20 位置問真的 git（未改）：`<真 git> <呼叫者的全域選項> rev-parse --absolute-git-dir --git-common-dir --show-toplevel --show-prefix`（同 cwd、同 `GIT_*`）；需要 git 2.20 以上（`config --worktree`）。代價：每次多一個 git 程序（約 +8 ms）。（使用者已追認 2026-09-25）

## 已知限制

shim 是安全帶，不是安全邊界。照[威脅模型](#威脅模型)，以下要刻意組出來、影響低，或 PATH shim／git 2.39 本來就看不到。

- **macOS login zsh 的 PATH 重排**：`/etc/zprofile` 的 `path_helper` 在 agent 的工具用 login shell（`/bin/zsh -lc`）跑指令時，會把系統路徑排到 `$AGEND_HOME/bin` 前面，shim 因此可能被排到後面（不是刻意繞過，是 macOS 的預設行為）。第 7 施工關 codex driver 的提案 P4 選了方案 A（agent 環境加 `ZDOTDIR=$AGEND_HOME/zsh`，在 `path_helper` 之後把 `$AGEND_HOME/bin` 重新排到最前面，一併修改第 6 施工關 H3 的環境白名單）；同樣的重排會發生在任何用 login shell 跑指令的 agent 工具（例如 claude 的 Bash 工具），要在第 12 施工關實測（見 [第 7 施工關 P4](gate-07-codex.md#p4codex-的啟動設定與-shim-會不會被繞過安全決定請明確選)）。
...
- `git config` 寫入會進共用的 repo config，影響 canonical checkout（例如 `git config core.hooksPath .githooks`、`user.email`、`remote.origin.url`）；`git remote` 的寫入改成拒絕（第 11 輪，見[範圍](#範圍)），`git config remote.<name>.*` 沒擋；agent worktree 的 `config.worktree` 優先，agend hook 不受影響，而且照樣串接到新設的 hook 目錄（hook 執行時才查）。
...
- 只有從自己的 workspace（`<AGEND_HOME>/workspace/<instance>` 或它底下，比對 canonical 路徑；本身不在任何 repo）沒帶 `-C`／`--git-dir`／`--work-tree`／`GIT_DIR`／`GIT_WORK_TREE` 的呼叫會導向，跑在 worktree 頂層：`.` 指 worktree 頂層（真 git 會說 `not a git repository`；會印出 `running in your bound worktree` 那行，破壞性的先快照）。其他不在任何 repo 的目錄（`/tmp/x`、`~`），以及帶了上面那些而 git 在那裡找不到 repo 時，不導向：寫入拒絕、讀取由 git 自己報錯（T6）。
