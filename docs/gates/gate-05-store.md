...
| S19 | 建新 DB 前，留下的 `.agend.db.new`（與它的 `-journal`）若是 home 擁有者的一般檔案就刪掉、從頭重建，不再「下次開啟接著建」；是 symlink、目錄或別人的檔案 → 拒絕並指名路徑。建立者之間靠 build 檔的 EXCLUSIVE lock 排隊：刪舊檔前先鎖它、鎖到新檔建好鎖好才放；鎖完再比對 path 上的 inode 是不是自己建的，不是就回 `InUse`；link 後在同一把鎖下刪掉 `.new` 這個名字 | verifier 第 2 輪：留下的 `.new` 被原樣 link 進來，0644 會變成 0644 的 `agend.db`、版本 5 變成 `TooNew`、symlink 會讓 `agend.db` 指到 home 外。新 home 的建立被殺不會丟使用者資料（從沒發佈過），重建永遠安全 | 改回接著建：`remove_leftover_build` 換回 `create(true)`＋`truncate(false)` 開舊檔 | 待追認 |
| S20 | `hard_link` 失敗（`AlreadyExists` 以外的任何錯誤，不只 ENOTSUP／EPERM）→ 再確認 `agend.db` 不存在後 `rename(.agend.db.new → agend.db)`；錯誤訊息寫出失敗的步驟（`creating agend.db failed: renaming … (hard-linking it failed first: …): …`）。rename 會蓋掉已存在的檔案，所以「確認後 rename」不能跟別的建立者搶：發佈的人一定持有 build 檔的 EXCLUSIVE lock（S19），同一時間只有一個；剩下的窗口只有 agend 以外的程序剛好在那一瞬間建 `agend.db` | verifier 第 2 輪：ExFAT 上新 home 永遠建不起來（os error 45）還留下 `.new`。錯誤碼各平台不同（macOS ENOTSUP=45、Linux EPERM／EOPNOTSUPP=95），std 沒有對應的 `ErrorKind`；其他錯誤 rename 也會失敗、訊息兩個都寫，所以一律退回比列錯誤碼簡單。測試用 `cfg(test)` 的 thread-local 讓 `hard_link` 失敗，不需要 ExFAT 映像 | 只對特定錯誤碼退回：`publish` 的 match 加一個 errno 表 | 待追認 |
| S21 | 開啟時檢查紀錄的 schema 版本該有的表都在：把前 N 個 migration 跑在 in-memory DB 上得到表名清單，跟 `agend.db` 的 `sqlite_master` 比；缺任何一張 → `MissingTables` 拒絕（訊息列出缺的表），檔案不動。只比表名，不比欄位與 index | verifier 第 2 輪：版本 1、沒有表的 DB 能開，每份每日快照擠掉一份好的。golden `schema.sql` 只描述最新版；重跑 migration 對每個版本都對，而測試已鎖定「migration 跑完＝golden」 | 改成讀 golden：`include_str!` golden 並解析 `CREATE TABLE`，只能檢查最新版 | 待追認 |
| S22 | SQLite 開檔前先檢查既有的 `agend.db`：比 SQLite 檔頭短（< 100 bytes）→ `Empty`（訊息寫實際 byte 數，0 bytes 維持原訊息）；指向不存在檔案的 symlink → `Refused`「it is a symlink to a missing file」；不是一般檔案（目錄等）→ `Refused`「it is not a regular file」。都不動檔案。README 加一句：刪掉 `agend.db` 等於從空 DB 開始、快照照常輪替 | verifier 第 2 輪：1 byte 的 `agend.db` 被拒，但 SQLite 先把它寫成 4096 bytes（unix VFS 把 1 byte 當 0）；dangling symlink 回 `File exists (os error 17)` 還留下建好的 `.new`。開檔前的大小檢查比事後說明簡單 | 拿掉大小檢查、在 README 說明：`check_existing` 改回只擋 0 bytes | 待追認 |
| S23 | DB 沒有任何 task 也沒有任何事件時，`snapshot` 不寫每日快照（`SnapshotReport.empty = true`、`taken = false`），輪替因此也不刪任何一份；升級前快照照做 | verifier 第 2 輪：刪掉 `agend.db`（S17 照設計重建空 DB）後，空 DB 的每日快照 7 天擠掉 7 份好的。空 DB 沒有資料可保護，跳過就讓舊快照留到真的有新資料為止 | 拿掉 `snapshot::daily` 的 `empty` 判斷與報告欄位 | 待追認 |
