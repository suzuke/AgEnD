...
4. 故意弄壞：把 `config.toml` 裡 Telegram 的 allowlist 改成空的（開工時細化：確切鍵名；目前沒有清空 allowlist 的命令），然後跑 doctor。
   ...
   應該看到：Telegram 那一行是失敗：allowlist 是空的、所有訊息會被丟棄，並附手動修正方式：在 `config.toml` 的 allowlist 加回你的 chat id（鍵名開工時細化）；`exit=1`（v1 #2207 的情境）。第 13 關之後，doctor 改為指向 `agend telegram setup`。
