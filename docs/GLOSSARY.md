...
| git fixture | git fixture | `git_fixture::GitFixture` | 測試用的暫存 git repo：canonical repo（`main` 有初始 commit）、bare team origin、linked worktree 與 branch，全在一個新的暫存目錄裡；指令的工作目錄必須嚴格在那個目錄裡面（目錄本身也拒絕），`git` 不收 `-C`、`--git-dir`、`-c` 這類全域選項，`commit` 不寫穿 symlink（子指令後面的路徑參數不檢查），並清掉繼承的 `GIT_*`／`AGEND_*` 與全域 git 設定。 | 契約 fixture；binding 快照 | [agend-testkit](../crates/agend-testkit/README.md#git-fixture) |
