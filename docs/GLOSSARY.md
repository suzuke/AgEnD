...
| change id／`{pr}` | change id／`{pr}` placeholder | `SubmittedChange::id`、`PipelineEvent::Submitted { change_id }`、`PipelineState::change_id`、`CommandContext::pr` | submit 關卡從 forge 拿回的變更編號（例如 PR 編號）；`command` 的 `{pr}` 佔位符展開成它（加單引號）。forge local 沒有 change id，所以用到 `{pr}` 的 command 在 local 下會讓 task 失敗。 | head（commit）；task id | D29、[pipeline](architecture/pipeline.md#6-種關卡) |
...
| `daemon_probe` | `daemon_probe` | `agend-daemon` example `daemon_probe` | 開發用的 example：daemon 沒跑時直接開 DB，`add`／`remove`／`list` instance；不是 production 命令，第 9 施工關才有 `agend instance add`。 | `holder_probe`（agend-holder 的 example，讀 holder 的畫面快照） | [第 6 施工關 P2](gates/gate-06-daemon-holder.md#p2instance-存哪開機怎麼決定要做什麼) |
| 孤兒 holder | orphan holder | `boot::BootAction::Orphan`（之後送 `HolderRequest::Shutdown`）（`plan_boot` 的輸出） | 開機時 `run/holders/` 有鎖檔、但 DB 裡已經沒有對應 instance 的 holder；daemon 直接送協定 `Shutdown`，不接回。 | **failed** 的 instance（holder 留著給人看，不算孤兒） | [第 6 施工關 P2](gates/gate-06-daemon-holder.md#p2instance-存哪開機怎麼決定要做什麼) |
| instance 狀態 | instance status | `store` 的 `instances` 表 | `new`（session 還沒建立，啟動帶 `--session-id`）、`running`（第一次 `Spawn` 已被 holder 確認，之後啟動帶 `--resume`）、`failed`（10 分鐘內死 3 次，daemon 停止重起、交給人；holder 留著看畫面）。 | 送達狀態（`queued → sent → confirmed／failed`，訊息的狀態，不是 instance 的） | [第 6 施工關 P6、H1](gates/gate-06-daemon-holder.md#p6holder-或-agent-死掉之後) |
| `logs/` | daemon log directory | `$AGEND_HOME/logs/daemon-YYYY-MM-DD.log` | daemon 自己寫的 log，每天一個檔，留 7 天；同目錄的 audit（`shim.jsonl`）另外輪替、留 14 天。 | `run/holders/<id>.log`（holder 自己的 log，7 天沒動過才刪）；`backups/`（DB 快照） | [第 6 施工關 P8](gates/gate-06-daemon-holder.md#p8daemon-log-與輪替) |
...
| shim | git shim | crate `agend-shim` | 只放在 agent PATH 上的 git／kill 防護：導向 worktree、擋離開 branch 與自建 worktree、破壞性操作前快照；讀唯讀 binding 快照。protected ref 由 agend hook 守。 | 使用者自己的 git（shim 不改它） | D5、D6 |
...
| binding 快照 | binding snapshot | `agend_shim::binding::Snapshot` | daemon 為每個 agent 寫的唯讀 JSON `$AGEND_HOME/bindings/<instance>.json`：instance、team repo、protected ref、目前的 binding；shim 只讀它，缺失或壞掉就拒絕寫入。 | binding 本身（存在 DB）；破壞性操作快照；螢幕快照 | D6、[第 3 施工關](gates/gate-03-shim.md#待你追認) |
| canonical checkout | canonical checkout | `Snapshot::source_repo`、`location::Location::Canonical` | team repo 的主要 clone，worktree 從它建立；agent 不改它。 | worktree；workspace | [第 3 施工關](gates/gate-03-shim.md#範圍) |
| 導向 | route | `classify::Decision::Run { route }` | shim 把 agent 在 worktree 外（workspace、canonical checkout、別的 worktree）跑的 git 改成在綁定的 worktree 執行。 | 拒絕；shim bypass | [第 3 施工關](gates/gate-03-shim.md#範圍) |
| protected ref | protected ref | `protected_ref::ProtectedRefs` | 只有 daemon 能寫的 ref：main、master，加上 binding 快照列的；綁定的 agent 也不能寫。 | merge 門檻 | [V1-LESSONS](V1-LESSONS.md) #13、[pipeline](architecture/pipeline.md#git-shimd5d6) |
| 破壞性操作快照 | pre-destructive snapshot | `snapshot::take`、`refs/agend/snapshots/<instance>/<id>` | `reset --hard`、`clean -f` 等執行前，shim 把整個 working tree（含未追蹤檔）存成 commit，並印出還原命令。 | binding 快照；螢幕快照；DB 每日快照 | [第 3 施工關](gates/gate-03-shim.md#範圍) |
| shim bypass | shim bypass | `AGEND_SHIM_BYPASS=1` | 讓 shim 不檢查、直接跑真的 git／kill；每次都記 audit。 | 導向 | [第 3 施工關](gates/gate-03-shim.md#待你追認) |
| audit 記錄 | audit record | `audit::Record`、`$AGEND_HOME/audit/shim.jsonl` | shim 的拒絕、bypass、快照，一行一筆 JSON。 | 進度紀錄；驗收紀錄 | [第 3 施工關](gates/gate-03-shim.md#範圍) |
...
| 契約測試 | contract suite | `contract::<trait>::run`、`contract::Report` | 一個 trait 的一組具名規則，對假實作與真實作跑同一份，讓假實作不會漂移；報表一行 `contract Forge: fake 11/11 pass`；規則編號見 [CONTRACTS.md](../crates/agend-testkit/CONTRACTS.md)。 | 單元測試；protocol golden 測試 | D9、v1 #1483、[agend-testkit](../crates/agend-testkit/README.md#契約測試怎麼接真實作) |
...
| git fixture | git fixture | `git_fixture::GitFixture` | 測試用的暫存 git repo：canonical repo（`main` 有初始 commit）、bare team origin、linked worktree 與 branch，全在一個新的暫存目錄裡；指令的工作目錄必須在 fixture 自己建的 repo（canonical、origin、linked worktree 或其子目錄）裡、不在 `.git` 裡（暫存目錄本身與 `worktrees/` 都拒絕），暫存目錄或 label 含 `:` 就不建，`git` 不收 `-C`、`--git-dir`、`-c` 這類全域選項，`commit` 不寫穿 symlink 或 hard link、不寫 `.git` 路徑（子指令後面的路徑參數不檢查），並清掉繼承的 `GIT_*`／`AGEND_*` 與全域 git 設定。威脅模型：防善意測試的失誤（路徑錯、`cd` 失敗、繼承的環境變數），不防故意改 git 內部檔案逃出的測試（同第 3 施工關 shim）；只支援 unix。 | 契約 fixture；binding 快照 | [agend-testkit](../crates/agend-testkit/README.md#git-fixture) |
