...
| 契約測試 | contract suite | `contract::<trait>::run`、`contract::Report` | 一個 trait 的一組具名規則，對假實作與真實作跑同一份，讓假實作不會漂移；報表一行 `contract Forge: fake 8/8 pass`。 | 單元測試；protocol golden 測試 | D9、v1 #1483、[agend-testkit](../crates/agend-testkit/README.md#契約測試怎麼接真實作) |
