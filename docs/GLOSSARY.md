...
| git fixture | git fixture | `git_fixture::GitFixture` | 測試用的暫存 git repo：canonical repo（`main` 有初始 commit）、bare team origin、linked worktree 與 branch，全在一個新的暫存目錄裡；拒絕在那個目錄外跑任何指令，並清掉繼承的 `GIT_*`／`AGEND_*` 與全域 git 設定。 | 契約 fixture；binding 快照 | [agend-testkit](../crates/agend-testkit/README.md#git-fixture) |
