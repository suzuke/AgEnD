...
| attempt | attempt | `PipelineState::attempt`、action 與事件的 `attempt` 欄位 | task 進入某個關卡的次數（第一次是 1；返工、重跑 checks、fanout 重跑都加 1）；結果必須帶目前的 attempt。fanout 的 attempt 就是它這一輪的 run id。 | 重試次數；patch-id | [pipeline](architecture/pipeline.md#6-種關卡) |
