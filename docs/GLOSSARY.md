...
| 孤兒 holder | orphan holder | `boot::Action::Shutdown`（`plan_boot` 的輸出） | 開機時 `run/holders/` 有鎖檔、但 DB 裡已經沒有對應 instance 的 holder；daemon 直接送協定 `Shutdown`，不接回。 | **failed** 的 instance（holder 留著給人看，不算孤兒） | [第 6 施工關 P2](gates/gate-06-daemon-holder.md#p2instance-存哪開機怎麼決定要做什麼) |
